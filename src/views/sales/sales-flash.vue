...
//            {type: 'date', required: true, message: '请选择结束时间', trigger: 'blur'}
    ...
    methods: {
      ...
      // 获取限时抢购列表
      get_seles() {
        this.axios.post(this.getApi('seller/auth/flash_sale_list'),
          this.dafa({
            index: this.index,
      ...
      get_seles_goods() {
        ...
        this.axios.post(this.getApi('seller/auth/flash_sale_goods'),
          this.dafa({
      ...
      edit_seles(id = '') {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/flash_sale_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/flash_sale_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/flash_sale_save'),
                this.dafa({
                  id: this.id,
