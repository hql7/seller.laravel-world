  ...
  export default {
    data() {
      return {
        ...
        status: '', // 活动状态 0=尚未开始  1=进行中 2=已结束(按时间去查)
        search_str: '', // 搜索活动名
...
//            {required: true, type: 'date', message: '请选择结束时间', trigger: 'blur'}
    ...
    methods: {
      ...
      // 获取列表
      get_seles() {
        this.axios.post(this.getApi('seller/auth/prom_order_list'),
          this.dafa({
            index: this.index,
      ...
      edit_seles(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/prom_order_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/prom_order_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.post(this.getApi('seller/auth/prom_order_save'),
              this.dafa({
                id: this.id,
