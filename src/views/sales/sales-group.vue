  ...
  <div class="member-list prop-box">
    ...
    <div class="add-member prop" v-if="add_show">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px">
        ...
        <el-form-item label="团购活动允许购买的最大数量" prop="max_num">
          <el-input v-model.number="ruleForm.max_num"></el-input>
        </el-form-item>
        <el-form-item label="团购活动成立的最小购买量" prop="min_num">
          <el-input v-model.number="ruleForm.min_num"></el-input>
        </el-form-item>
  ...
  export default {
    data() {
      return {
        ...
        start_time: '', // 起始时间
        end_time: '', // 截止时间
        status: '', // 状态  0=尚未开始  1=进行中 2=已结束(按时间去查)
        search_str: '', // 搜索活动名
        ...
        rules: {
          ...
          max_num: [
            {required: true, type: 'number', message: '请设置团购活动允许购买的最大数量(数值)', trigger: 'blur'}
          ],
          min_num: [
            {required: true, type: 'number', message: '团购活动成立的最小购买量（数值）', trigger: 'blur'}
          ],
          price: [
            {required: true, type: 'number', message: '请输入团购价格（数值）', trigger: 'blur'}
          ],
          start_time: [
...
//            {type: 'date', required: true, message: '请选择结束时间', trigger: 'blur'}
    ...
    methods: {
      ...
      // 获取列表
      get_seles() {
        this.axios.post(this.getApi('seller/auth/groupbuy_list'),
          this.dafa({
            index: this.index,
      ...
      get_seles_goods() {
        ...
        this.axios.post(this.getApi('seller/auth/groupbuy_goods'),
          this.dafa({
      ...
      edit_seles(id = '') {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/groupbuy_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/groupbuy_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/groupbuy_save'),
                this.dafa({
                  id: this.id,
                  title: this.ruleForm.title,
                  max_num: this.ruleForm.max_num,
                  good_id: this.ruleForm.good_id,
                  min_num: this.ruleForm.min_num,
