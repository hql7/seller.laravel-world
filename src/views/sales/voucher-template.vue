  ...
  <div class="member-list prop-box">
    ...
    <div class="m-member-list m-prop-box">
      ...
      <h2>代金券列表
        ...
        <div class="u-right">
          <el-select class="sele-s" v-model="type" placeholder="代金券类型" :clearable="true">
            <el-option label="指定发放" value="1"></el-option>
            <el-option label="免费领取" value="2"></el-option>
            <el-option label="线下发放" value="3"></el-option>
          </el-select>
          <el-select class="sele-s" v-model="cate_id" placeholder="一级分类" :clearable="true">
            <el-option v-for="item in level1" :label="item.name" :value="item.id" :key="item.id"></el-option>
          </el-select>
    ...
    <div class="add-member prop" v-if="add_show">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px">
        ...
        <el-form-item label="一级商品分类" required>
          <el-select v-model="ruleForm.cate_id">
        ...
        <el-form-item label="可使用条件" v-if="ruleForm.use_type==2">
          <el-select v-model="lv1" @change="getLv2">
            <el-option v-for="item in level1" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
  ...
  export default {
    data() {
      return {
        ...
        search_str: '', // 搜索活动名
        cate_id: '', // 一级分类
        type: '', // 代金券类型  1=指定发放 2=免费领取 3=线下发放
...
//            {type: 'date', required: true, message: '请选择结束时间', trigger: 'blur'}
        ...
        choice_pgCount: 1, // 可选加入活动的商品列表总页数
        search_type: '', // 搜索类型
        search_str_: '', // 搜索关键词
    ...
    methods: {
      ...
      // 获取代金券列表
      get_voucher() {
        this.axios.post(this.getApi('seller/auth/voucher_template_list'),
          this.dafa({
            index: this.index,
      ...
      get_vip(val) {
        if (val) {
          this.axios.post(this.getApi("root/auth/vip_list")
          ).then((res) => {
            if (res.data.code === 0) {
      ...
      // 发放代金券页面
      send_voucher() {
        this.axios.post(this.getApi('seller/auth/voucher_template_send_page'),
          this.dafa({
            id: this.id,
            type: this.FF_type,
      ...
      get_seles_goods() {
        ...
        this.axios.post(this.getApi('seller/auth/voucher_search_goods'),
          this.dafa({
      ...
      edit_voucher(id = '') {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/voucher_template_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
              ...
              if (res.data.info) {
                ...
                this.level1.push({'id': res.data.info.cate_id, 'name': res.data.info.cate_name});
      ...
      del_voucher(id) {
        this.$confirm('是否确认删除?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/voucher_template_del'),
              this.dafa({
                id_list: id
              })
            ).then(res => {
      ...
      // 发放
      send_to_ajax(data) {
        this.axios.post(this.getApi('seller/auth/voucher_template_send'),
          data
        ).then(res => {
          this.$message(res.data.msg);
          if (res.data.code === 0) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/voucher_template_save'),
                this.dafa({
                  id: this.id,
