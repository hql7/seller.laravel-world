    ...
    <div class="m-member-list m-prop-box">
      ...
      <h2>促销活动列表
        ...
        <div class="u-right">
          ...
          <el-select class="sele-s" v-model="status" placeholder="活动状态" :clearable="true">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.type==0">直接打折</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
      ...
      <el-tabs type="border-card">
        <el-tab-pane label="基本信息">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px">
            ...
            <!--<el-form-item label="买就赠代金券" prop="expression" v-else-if="ruleForm.type==3">-->
            <!--<el-input v-model="ruleForm.expression"></el-input>-->
            <!--</el-form-item>-->
  ...
  export default {
    data() {
      return {
        ...
        start_time: '', // 起始时间
        end_time: '', // 截止时间
        status: '', // 0=尚未开始  1=进行中 2=已结束(按时间去查)
...
//            {required: true, type: 'date', message: '请选择结束时间', trigger: 'blur'}
    ...
    methods: {
      ...
      // 获取商品促销列表
      get_seles() {
        this.axios.post(this.getApi('seller/auth/prom_goods_list'),
          this.dafa({
            index: this.index,
      ...
      // 查看商品
      see_goods(id) {
        this.axios.post(this.getApi('seller/auth/prom_goods_view'),
          this.dafa({
            id: id,
      ...
      // 获取可参与活动商品
      get_seles_goods() {
        this.axios.post(this.getApi('seller/auth/can_prom_goods'),
          this.dafa({
      ...
      edit_seles(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/prom_goods_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/prom_goods_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/prom_goods_save'),
                this.dafa({
                  id: this.id,
