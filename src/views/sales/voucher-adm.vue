<template>
  <div class="goods-evaluate-list prop-box">
    ...
    <div class="m-goods-evaluate-list m-prop-box">
      ...
      <h2>代金券列表
        <el-button type="text" size="small" class="add-member-btn add-btn" @click="exportVoucher()">导出数据
        </el-button>
        <div class="u-right">
          ...
          <el-select class="sele-s" v-model="status" placeholder="代金券状态" :clearable="true">
            ...
            <el-option label="未领取" value="3"></el-option>
          </el-select>
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `优惠促销管理, 由平台设置管理。导出数据可以选择下方筛选项精准导出`
        }, // 提示模块参数
        tableData: [], // 表格数据
        disabled: '', // 是否禁用  0=正常 1=禁用
        status: '', // 代金券状态  0=未使用 1=已使用 2=已过期 3=未领取
    ...
    methods: {
      ...
      // 获取代金券列表
      get_voucher() {
        this.axios.post(this.getApi('seller/auth/voucher_list'),
          this.dafa({
            index: this.index,
            status: this.status,
            type: this.type,
      ...
      del_voucher(id, val) {
        this.$confirm('是否确认操作?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/voucher_act'),
              this.dafa({
                id_list: this.del_id,
                type: val
              })
      ...
      // 导出
      exportVoucher() {
        this.axios.post(this.getApi('seller/auth/voucher_export'),
          this.dafa({
            status: this.status,
            type: this.type,
            disabled: this.disabled,
