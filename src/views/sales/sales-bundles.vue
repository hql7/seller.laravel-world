...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取商品规格列表
      get_seles() {
        this.axios.post(this.getApi('seller/auth/bundling_list'),
          this.dafa({
            index: this.index,
      ...
      // 获取可参与活动商品
      get_seles_goods() {
        this.axios.post(this.getApi('seller/auth/bundling_goods'),
          this.dafa({
      ...
      edit_seles(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/bundling_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/bundling_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/bundling_save'),
                this.dafa({
                  id: this.id,
