      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <!--</el-button>-->
            <el-dropdown @command="handle($event,tableData[scope.$index].id,scope.$index,tableData)">
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].payment == 1 ">账户余额</span>
            <span v-else>其他</span>
          </template>
        </el-table-column>
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span class="c-danger" v-if="tableData[scope.$index].pay_status == 0 ">未付款</span>
            <span class="c-success" v-else-if="tableData[scope.$index].pay_status == 1 ">已付款</span>
            <span class="c-warning" v-else-if="tableData[scope.$index].pay_status == 2 ">申请退款</span>
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span class="c-warning" v-if="tableData[scope.$index].delivery_status == 0 ">未发货</span>
            <span class="c-blue" v-else-if="tableData[scope.$index].delivery_status == 1 ">已发货</span>
            <span class="c-success" v-else-if="tableData[scope.$index].delivery_status == 2 ">已签收</span>
            <span class="c-warning" v-else-if="tableData[scope.$index].delivery_status == 3 ">申请退货</span>
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span class="c-blue" v-if="tableData[scope.$index].status == 0 ">等待审核</span>
            <span class="c-blue" v-else-if="tableData[scope.$index].status == 1 ">等待审核</span>
            <span class="c-blue" v-else-if="tableData[scope.$index].status == 2 ">等待审核</span>
            <span class="c-success" v-else-if="tableData[scope.$index].status == 3 ">已审核</span>
            <span class="c-999" v-else-if="tableData[scope.$index].status == 4 ">已完成</span>
            <span class="c-999" v-else-if="tableData[scope.$index].status == 5 ">已取消</span>
...
//        ruleForm: {}, // 组织发货所需数据格式
    ...
    methods: {
      ...
      // 获取订单列表
      get_order() {
        this.axios.post(this.getApi('root/auth/order_list'),
          this.dafa({
            type: this.type,
            is_del: this.is_del,
            index: this.index,
      ...
      to_do_ok(formName) {
        // 编辑操作
        if (this.to_do == 2) {
          this.axios.post(this.getApi('root/auth/order_save'),
            this.dafa({
              id: this.id,
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              this.axios.post(this.getApi('root/auth/order_ship'),
                this.dafa({
                  order_id: this.id,
          ...
          if (this.remarks) {
            this.axios.post(this.getApi('root/auth/order_act'),
              this.dafa({
                id: this.id,
      ...
      // 查看订单信息
      see_oeder_info() {
        this.axios.post(this.getApi('root/auth/order_view'),
          `id=${this.id}`
        ).then(res => {
          if (res.data.code == 0) {
            // 商品信息
      ...
      del_order(val, index, rows) {
        ...
        this.$confirm(title, '提示', {
          ...
          // 恢复或删除
          if (val == 1) {
            this.axios.post(this.getApi('root/auth/order_recover'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
...
//            console.log(err)
      ...
      /*  submitForm (formName) {
       this.$refs[formName].validate((valid) => {
       if (valid) {
       this.axios.post(this.getApi('root/auth/specs_save'),
       this.dafa({
       id: this.id,
