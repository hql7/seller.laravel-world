      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="danger" size="small" icon="el-icon-search" @click="see_order(scope.$index, tableData)">查看
            </el-button>
          </template>
        </el-table-column>
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].doc_type == 1 ">充值</span>
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].pay_status == 1 ">已付款</span>
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <span v-if="tableData[scope.$index].type == 1 ">团购订单</span>
            <span v-else-if="tableData[scope.$index].type == 2 ">限时抢购</span>
            <span v-else-if="tableData[scope.$index].type == 3 ">捆绑促销</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取收款单列表
      get_order_receipt() {
        this.axios.post(this.getApi('root/auth/receiving_list'),
          this.dafa({
            index: this.index,
      ...
      // 订单操作
      see_order(index, rows) {
        this.dialogVisible = true;
        this.axios.post(this.getApi('root/auth/receiving_view'),
          `id=${rows[index].id}`
        ).then(res => {
          if (res.data.code == 0) {
      ...
      // 获取支付方式
      get_payment() {
        this.axios.post(this.getApi('root/auth/payment_list')
        ).then(res => {
          if (res.data.code == 0) {
