  ...
  <div class="type-list prop-box">
    ...
    <div class="add-type prop" v-if="add_show">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px" class="demo-ruleForm">
        ...
        <el-form-item label="是否开启店铺满额包邮">
          <el-switch
            v-model="ruleForm.is_free_ship"
            active-color="#13ce66"
            inactive-color="#ccc"
            active-text="是"
            inactive-text="否"
        ...
        <el-form-item prop="ship_money" label="店铺包邮需满金额（元）" v-if="ruleForm.is_free_ship == 1">
          <el-input v-model.number="ruleForm.ship_money"></el-input>
        </el-form-item>
        <el-form-item label="默认运费">
          <el-col :span="5">
            <el-form-item prop="first_weight">
              <el-tooltip content="填写首重" placement="top">
          ...
          <el-col :span="5">
            <el-form-item prop="first_price">
              <el-tooltip content="填写首重运费" placement="top">
          ...
          <el-col :span="5">
            <el-form-item prop="second_weight">
              <el-tooltip content="填写续重" placement="top">
          ...
          <el-col :span="5">
            <el-form-item prop="second_price">
              <el-tooltip content="填写续重运费" placement="top">
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `上传商品时，若选择不包邮则按运费模版计算运费，运费模版可设置店铺满额包邮，指定地区邮费，首重运费续重运费。`
        }, // 提示模块参数
        tableData: [], // 表格数据
    ...
    methods: {
      ...
      // 获取
      get_system() {
        this.axios.post(this.getApi('seller/auth/fare_list')
        ).then(res => {
          if (res.data.code === 0) {
      ...
      edit_system(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/fare_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      edit_city_yf(id, index) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/fare_show_area'),
            this.dafa({
      ...
      set_system(id) {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/fare_use'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            // this.$message(res.data.msg);
      ...
      del_system(id) {
        this.$confirm('是否确认删除?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/fare_del'),
              `id=${id}`
            ).then(res => {
              resolve(res);
              // this.$message(res.data.msg);
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/fare_save'),
                this.dafa({
                  id: this.ruleForm.id,
                  name: this.ruleForm.name,
                  is_free_ship: this.ruleForm.is_free_ship,
                  ship_money: this.ruleForm.ship_money,
