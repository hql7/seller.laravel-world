    ...
    <div class="m-order-list m-prop-box">
      ...
      <h2 class="large-search">
        <div class="u-right">
          ...
          <el-select class="sele-s" v-model.number="payment" placeholder="支付方式" :clearable="true">
            <el-option v-for="item in payment_list" :label="item.name" :value="item.id"
          ...
          <el-select class="sele-s" v-model="delivery_status" placeholder="配送状态" :clearable="true">
            ...
            <el-option label="售后中" value="3"></el-option>
            <el-option label="售后完成" value="4"></el-option>
          </el-select>
      ...
      <el-table
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown @command="handle($event,scope.row.id)">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="0"><i class="iconfont icon-chakan-copy"></i>&nbsp;查看
                </el-dropdown-item>
                ...
                <el-dropdown-item command="5"><i class="iconfont icon-beizhu"></i>&nbsp;备注
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span class="c-success" v-else-if="scope.row.pay_status == 2 ">申请退款</span>
            <span class="c-warning" v-else>已退款</span>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span class="c-warning" v-else>售后完成</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
          ...
          min-width="100">
          <template slot-scope="scope">
            <span class="c-success" v-if="scope.row.status == 3 ">已审核</span>
            <span class="c-999" v-else-if="scope.row.status == 4 ">已完成</span>
            <span class="c-999" v-else-if="scope.row.status == 5 ">已取消</span>
            <span class="c-999" v-else-if="scope.row.status == 6 ">已作废</span>
            <s class="c-danger" v-else>等待审核</s>
          </template>
        </el-table-column>
        <!--打印暂时搁置-->
      </el-table>
      ...
      <div class="see" v-if="to_do==0 || to_do==2 || to_do==1">
        ...
        <div class="fa-huo" v-if="to_do ==1">
          ...
          <el-form :model="ruleForm" :rules="rules" :show-message="false" ref="ruleForm" label-width="120px"
            ...
            <el-form-item label="物流公司：" prop="express_company_id">
              <el-select v-model="ruleForm.express_company_id" placeholder="请选择物流">
                <el-option v-for="item in express" :label="item.name" :key="item.id"
            ...
            <el-form-item label="收货地区：">
              <el-col :span="7">
                <el-form-item prop="province">
                  <el-select v-model="ruleForm.province" filterable placeholder="选择省份"
              ...
              <el-col :span="7">
                <el-form-item prop="city">
                  <el-select v-model="ruleForm.city" filterable placeholder="选择城市"
                             @change="get_county">
                    <el-option
                      v-for="item in city"
                      :key="item.id"
        ...
        <div class="m-see" v-else>
          ...
          <ul class="order-info order-info-ul">
            ...
            <li><span>佣金（平台抽成）：</span>{{order_info.handling_fee}}</li>
            ...
            <li class="el-ipt-m"><span>订单折扣或涨价金额：</span>
              <el-tooltip class="item" effect="dark" content="例：想给顾客降价10元，就输入-10；想涨价10元，就输入10"
                          placement="top-start">
                <el-input :disabled="checkLookOrShip"
                          v-model="order_info.adjust_amount"
                          @blur="change_price(order_info.adjust_amount)"
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `订单列表显示所有类型订单，但一般常用处理操作为发货，退款退换信息请至售后服务处理
          点击处理按钮将显示查看订单等（包括订单物品）的详细信息。导出数据时选择下方时间范围，
          支付方式，订单状态，支付状态，配送状态可以按条件导出。`
        }, // 提示模块参数
        totalPage: 1, // 总页
        tableData: [], // 表格数据
        pickerOptions: {
        ...
        id: '', // 所编辑id
        del_id: [], // 删除储存数组
        status: this.$route.params.status, // 订单状态  0=1=2=等待审核 3=已审核 4=已完成 5=已取消 6=已作废
        delivery_status: this.$route.params.delivery_status, // 配送状态  0=未发货 1=已发货 2=已签收 3=申请退货 4=已退货 5=申请换货 6=已换货
        loading: true
      }
    },
    created() {
    ...
    methods: {
      ...
      // 获取订单列表
      get_order() {
        this.axios.post(this.getApi('seller/auth/order_list'),
          this.dafa({
            index: this.index,
      ...
      see_oeder_info() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/order_view'),
            `id=${this.id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      edit_order() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/order_save'),
            this.dafa({
              id: this.id,
      ...
      order_send(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/order_ship'),
                this.dafa({
                  order_id: this.id,
      ...
      order_act() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/order_act'),
            this.dafa({
              id: this.id,
      ...
      // 获取支付方式
      getPayment() {
        this.axios.post(this.getApi('coms/pay_type'),
          `type=2`
        ).then(res => {
          if (res.data.code === 0) {
            this.payment_list = res.data.pay_type
          }
        })
      },
      // 改价
      ...
      // 获取快递公司列表
      get_express() {
        this.axios.post(this.getApi('coms/express_company_list')
        ).then(res => {
          if (res.data.code === 0) {
      ...
      /*del_order(id) {
        this.$confirm('是否确认将此订单放入回收站?', '提示', {
          ...
          this.axios.post(this.getApi('root/auth/order_del'),
            this.dafa({
              id_list: this.del_id,
              real_del: 0
            })
      ...
      // 导出数据
      out_data() {
        this.axios.post(this.getApi('seller/auth/order_export'),
          this.dafa({
            start_time: this.start_time,
            end_time: this.end_time,
            payment: this.payment,
