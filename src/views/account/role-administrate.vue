...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取角色列表
      get_roles() {
        this.axios.post(this.getApi('seller/auth/roles')
        ).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
      ...
      edit_role(id = '') {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/roles-edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_role(id) {
        this.$confirm('是否确认删除角色?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/roles-del'),
              this.dafa({
                id: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/roles-save'),
                this.dafa({
                  id: this.id,
