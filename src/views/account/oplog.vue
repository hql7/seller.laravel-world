<template>
  <div class="oplog prop-box">
    ...
    <div class="m-oplog">
      ...
      <h2>操作日志列表<span>({{tableData.length}})条</span>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button v-if="_super == 1" type="danger" size="small" icon="delete"
                       @click="del_role(scope.row.id)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取
      get_roles(search) {
        this.axios.post(this.getApi('seller/auth/log_list'),
          this.dafa({
      ...
      del_role(id) {
        this.$confirm('是否确认删除?', '提示', {
          ...
          this.axios.post(this.getApi('seller/auth/log_del'),
            this.dafa({
              id_list: this.del_id
            })
          ).then(res => {
