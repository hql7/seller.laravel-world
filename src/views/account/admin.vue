    ...
    <div class="m-admin m-prop-box">
      ...
      <h2>资源权限列表<span>（共{{tableData.length}}条记录）</span>
        ...
        <!-- <el-input placeholder="按管理员名称搜索" v-model="search" class="u-right search">
           &lt;!&ndash;<template slot="append" @click="get_admins(true)">搜索</template>&ndash;&gt;
    ...
    <div class="add-admin prop" v-if="add_show">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" lalabel-position="right" label-width="240px">
        <el-form-item label="管理员名称" prop="name">
          <el-input v-model="ruleForm.name" :disabled="admin_name_disabled" placeholder="请填写管理员名称"></el-input>
        </el-form-item>
        <el-form-item label="账号级别" prop="pass">
          <template v-if="ruleForm.super == 1">店主</template>
          <template v-else>店铺管理</template>
          <!--<el-select v-model.number="ruleForm.super" placeholder="请选择">
            <el-option label="店铺管理" :value="0"></el-option>
            <el-option label="店主" :value="1"></el-option>
          </el-select>-->
        </el-form-item>
        <el-form-item label="原密码" prop="pass" v-if="admin_name_disabled">
          <el-input v-model="ruleForm.old_pass" type="password" placeholder="请填写登录密码"></el-input>
        ...
        <el-form-item label="确认新密码" prop="ckPass">
          <el-input v-model="ruleForm.ckPass" type="password" placeholder="请确认登录密码"></el-input>
        </el-form-item>
        <template v-if="ruleForm.super == 0">
          <el-form-item label="账号状态" prop="is_lock">
            <el-select v-model="ruleForm.is_lock" placeholder="请选择所属角色">
  ...
  export default {
    data() {
      ...
      return {
        hint: {
          title: '操作提示',
          content: `网站系统管理员, 由总平台设置管理.点击列表里的删除按钮删除单个管理员时不受复选框的影响！复选框数据只在点击列表下方批量删除时有效。锁定管理员之后将不能登录。`
        }, // 提示模块参数
        tableData: [], // 表格数据
        search: '', // 搜索数据
        ...
        admin_name_disabled: false, // 是否禁改管理员名
        id: '', // 所编辑id
        del_id: [], // 删除储存数组
        loading: true
      }
    ...
    methods: {
      ...
      go_back() {
        this.add_show = false;
        this.admin_name_disabled = false;
      },
      // 请求管理员列表
      get_admins() {
        this.axios.post(this.getApi('seller/auth/admins')
        ).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
      ...
      edit_admin(id = '') {
        if (id) {
          this.admin_name_disabled = true;
        }
        this.id = id;
        this.add_show = true;
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/admin-edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/admins-save'),
                this.dafa({
                  id: this.id,
                  name: this.ruleForm.name,
                  old_pass: this.ruleForm.old_pass,
                  new_pass: this.ruleForm.pass,
                  is_lock: this.ruleForm.is_lock,
      ...
      del_admin(id) {
        this.$confirm('是否确认删除管理员?', '提示', {
          ...
          this.axios.post(this.getApi('seller/auth/admins-del'),
            this.dafa({
              id: this.del_id
            })
          ).then(res => {
