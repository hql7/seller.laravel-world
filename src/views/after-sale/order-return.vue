  ...
  <div class="order-receipt-list prop-box">
            ...
            <el-dropdown
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item command="4"
                                  v-if="tableData[scope.$index].status == 6 && tableData[scope.$index].after_type != 0">
                ...
                <el-dropdown-item command="6"><i class="iconfont icon-wancheng"></i>售后记录
                </el-dropdown-item>
                <el-dropdown-item command="7"><i class="iconfont icon-wancheng"></i>物流信息
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
    ...
    <el-dialog
      ...
      <div class="see" v-if="val == 0 || val == 1">
        ...
        <h3>退换货维修单详情：</h3>
        <ul class="order-info order-info-ul">
          <li><span>订单类型：</span><em v-if="order_info.type==3">捆绑促销</em><em
            v-else-if="order_info.type==1">团购订单</em><em
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取退换货维修单列表
      get_order_return() {
        this.axios.post(this.getApi('seller/auth/return_list'),
          this.dafa({
            index: this.index,
      ...
      // 售后记录
      post_sale() {
        this.axios.post(this.getApi('seller/auth/after_process'),
          `id=${this.id}`,
        ).then(res => {
          if (res.data.code === 0) {
      ...
      // 物流信息
      logis_infor() {
        this.axios.post(this.getApi('seller/auth/express_info'),
          `id=${this.id}`,
        ).then(res => {
          if (res.data.code === 0) {
      ...
      see_order() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/return_view'),
            `id=${this.id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
              ...
              if (res.data.after_info.seller_info) {
                ...
                this.province_id = this.seller_info.province_id;
                this.province = [{id: this.seller_info.province_id, name: this.seller_info.province_name}];
                this.city_id = this.seller_info.city_id;
                this.city = [{id: this.seller_info.city_id, name: this.seller_info.city_name}];
                this.county_id = this.seller_info.county_id;
                this.county = [{id: this.seller_info.county_id, name: this.seller_info.county_name}];
                this.addr = this.seller_info.addr;
              } else {
                this.seller_info = {
                  seller_name: '',
                  location: '',
      ...
      sendHandle(val) {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/return_verify'),
            this.dafa({
              id: this.id,
              result: val,
      ...
      handle_2_3() {
        if (this.handling_idea) {
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/return_receipt'),
              this.dafa({
                id: this.id,
                type: this.val == 2 ? 1 : 2,
      ...
      return_good() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/return_send_page'),
            `id=${this.id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      to_return_good() {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/return_send_act'),
            this.dafa({
              /*         id: this.id,
              ...
              province: this.accept_info.province_id,
              city: this.accept_info.city_id,
              county: this.accept_info.county_id,
      ...
      ret_ok() {
        this.$prompt('请输入处理意见', '提示', {
          ...
          this.axios.post(this.getApi('seller/auth/return_refund'),
            this.dafa({
              id: this.id,
              handling_idea: value
            })
