        ...
        <ul class="order-info order-info-ul">
          ...
          <li class="textarea-box"><span>处理意见：</span>
            <el-input class="adjust_note" type="textarea"
                      :rows="2" v-model="refund_info.handling_idea" placeholder="请输入内容"></el-input>
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取退款单列表
      get_order_refund() {
        this.axios.post(this.getApi('seller/auth/refund_list'),
          this.dafa({
            index: this.index,
      ...
      see_order(id, status) {
        ...
        this.id = id;
        status == 0 ? this.see = false : this.see = true;
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/refund_view'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      // 处理退款单
      handle_refund(val) {
        if (this.refund_info.handling_idea) {
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/refund_act'),
              this.dafa({
                id: this.id,
                status: val,
                handling_idea: this.refund_info.handling_idea
              })
            ).then(res => {
              resolve(res);
              // this.$message(res.data.msg);
