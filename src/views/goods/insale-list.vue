  ...
  <div class="goods-list prop-box">
    ...
    <div class="m-goods-list m-prop-box">
      ...
      <h2>在售商品列表
        ...
        <div class="u-right">
          <el-select class="sele-s" clearable v-model="shop_cate" placeholder="店铺分类">
            <el-option label="请选择" value="" disabled></el-option>
            <el-option
              v-for="item in shop_cate_list"
              :key="item.id"
              :label="item.name"
              :value="item.id">
            </el-option>
          </el-select>
          <el-select class="sele-s" clearable @change="getFL($event,1,'s')"
          ...
          <el-select class="sele-s" clearable v-model="category_id2" placeholder="二级分类">
            <!--<el-option label="请选择" value="" disabled></el-option>-->
            <el-option
          ...
          <el-select class="sele-s" clearable v-model="search_type" placeholder="搜索类型">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-switch
              ...
              @change="stateOperation($event,0,scope.row.id)"
              active-text="在售"
              inactive-text="下架"
              active-color="#13ce66"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-switch
              ...
              @change="stateOperation($event,1,scope.row.id)"
              active-text="是"
              inactive-text="否"
              active-color="#13ce66"
              inactive-color="#ff4949"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-switch
              ...
              @change="stateOperation($event,2,scope.row.id)"
              active-text="是"
              inactive-text="否"
              active-color="#13ce66"
              inactive-color="#ff4949"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.is_commend"
              @change="stateOperation($event,3,scope.row.id)"
              active-text="是"
              inactive-text="否"
              active-color="#13ce66"
              inactive-color="#ff4949"
        ...
        <el-table-column
          ...
          width="100">
          <template slot-scope="scope">
            <el-input v-model="scope.row.sort" @change="changeSort($event,scope.row.id)"></el-input>
          </template>
        </el-table-column>
        <el-table-column
          prop="operation"
      ...
      <el-tabs type="border-card">
        <el-tab-pane label="基本信息">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px">
            ...
            <el-form-item label="经营类目商品" required class="col_box">
              <el-select v-model="ruleForm.shop_fid_id" @change="getBrands" placeholder="请选择经营类目">
                <el-option v-for="item in businessCategory" :key="item.id" :label="item.fname"
              ...
              <el-col :span="4">
                <el-form-item prop="goods_cly_3" class="el-form-item-no">
                  <el-tooltip class="item" effect="dark" content="最多成为第三级,如果要设置为第二级, 只选择第一级即可"
                              placement="top-start">
                    <el-select class="sele-s" v-model="ruleForm.goods_cly_3" @change="getBrands" placeholder="三级分类">
                      <el-option label="请选择" value="" disabled></el-option>
            ...
            <el-form-item label="本店商品分类" required class="col_box">
              <el-col :span="4">
                <el-form-item prop="shop_cate_1" class="el-form-item-no">
                  <el-select class="sele-s" @change="changeShopCate"
                             v-model="ruleForm.shop_cate_1"
                             placeholder="一级分类">
                    <el-option label="请选择" value="" disabled></el-option>
                    <el-option v-for="item in shop_cate_list" :key="item.id" :label="item.name"
                               :value="item.id"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="4">
                <el-form-item prop="shop_cate_2" class="el-form-item-no">
                  <el-select class="sele-s" v-model="ruleForm.shop_cate_2"
                             placeholder="二级分类">
                    <el-option label="请选择" value="" disabled></el-option>
                    <el-option v-for="item in shop_cate_list2" :key="item.id" :label="item.name"
                               :value="item.id"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            ...
            <el-form-item label="商品类型">
              <el-select v-model="ruleForm.product_type" @change="change_type" placeholder="请选择商品类型">
            ...
            <!--<div class="point-box">
              <h3>
                &lt;!&ndash; <el-tooltip class="item" effect="dark" content="购买此商品会赠送多少积分" placement="top-start">
                   <span>赠送积分</span>
                 </el-tooltip>&ndash;&gt;
              ...
              <h5>
                &lt;!&ndash;<span><el-input v-model="ruleForm.detail.point"></el-input></span>&ndash;&gt;
            ...
            <el-form-item label="可用积分">
              <el-input v-model="ruleForm.detail.exchange_point"></el-input>
            </el-form-item>
            <el-form-item label="计量单位">
              <el-input v-model="ruleForm.detail.unit"></el-input>
            </el-form-item>
            <!--属性区改造 多店铺适用 -- 结束-->
            <el-form-item label="商品展示图片">
              ...
              <el-dialog v-model="dialogVisible">
                <img width="100%" :src="dialogImageUrl" alt="商品图">
              </el-dialog>
        ...
        <el-tab-pane label="描述信息">
          ...
          <editor-item @getEditor="getEditor" :content="content_info"></editor-item>
        </el-tab-pane>
        ...
        <el-tab-pane label="售后服务">
          ...
          <editors-item @getEditor="getEditorS" :content="content_after_sale"></editors-item>
        </el-tab-pane>
      </el-tabs>
    </div>
    <!--// 开启规格-->
    <el-dialog title="选择规格" :visible.sync="open" width="80%">
      ...
      <table class="table">
        ...
        <tr>
          <td align="right"></td>
          <td class="specs-sc">
            <p class="c_a4 c-danger">下表的零售价是最终展示给买家的产品价格,重量是非包邮时计算邮费的依据，具体可在【运费设置】中设定。</p>
...
<script>
  ...
  import EditorsItem from '@/components/editorS.vue'
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `商品只能添加在店铺二级分类上，请先保证店铺至少有一个二级分类！->在店铺管理-店铺分类！违规下架，当商品处于违规下架状态时，前台将不能购买该商品，只有管理员可控制商品违规下架状态，并且商品只有重新编辑后才能上架。网站系统管理员, 由总平台设置管理.点击列表里的删除按钮删除单个管理员时不受复选框的影响！复选框数据只在点击列表下方批量删除时有效。商品名称进行搜索时为关键字搜索，货号必须全名称搜索！`,
        }, // 提示模块参数
        category_id1: '', // 一级分类
        classify1: [], // 二级分类
        category_id2: '', // 二级分类
        classify2: [], // 二级分类
        shop_cate: '', // 本店分类 - 搜索
        shop_cate_list: [], // 本店分类盒子
        shop_cate_list2: [], // 本店2级分类盒子
        search_type: '',
        search: '', // 搜索内容
        tableData: [], // 表格数据
        ...
        rules: {
          ...
          shop_cate_2: [
            {required: true, type: 'number', message: '请选择本店二级分类', trigger: 'blur'}
          ],
          'detail.goods_no': [
    ...
    methods: {
      ...
      // 监听子组件 编辑器内容
      getEditor(data) {
        this.content_info = data
      },
      // 监听子组件 编辑器内容 售后
      getEditorS(data) {
        this.content_after_sale = data
      },
      // 获取商品列表
      get_goods(search) {
        this.axios.post(this.getApi('seller/auth/goods'),
          this.dafa({
            type: 0, // 在售商品列表
            index: this.index,
            category_id: this.category_id2 ? this.category_id2 : this.category_id1,
            shop_cate: this.shop_cate,
      ...
      getBusinessCategory() {
        this.axios.post(this.getApi('seller/auth/shop_fid_list'),
          ...
          if (res.data.code === 0) {
            this.businessCategory = res.data.list
          }
        }).catch(err => {
      ...
      edit_good(id = '') {
        this.antiShake.antiShake((resolve, reject) => {
          this.id = id;
          this.axios.post(this.getApi('seller/auth/goods_edit'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
              this.add_show = true;
              if ('good_id' in res.data.info) {
                // 处理商品类型
                this.ruleForm.goods_type = res.data.info.goods_type;
                // 经营类目
                this.ruleForm.shop_fid_id = res.data.info.shop_fid_id;
                ...
                // 处理本店分类
                this.ruleForm.shop_cate_1 = res.data.info.cate1[0].id;
                this.shop_cate_list2 = res.data.info.cate2;
                this.ruleForm.shop_cate_2 = res.data.info.cate2[0].id;
                // 处理商品类型
                ...
                // 处理商品的描述信息
                this.content_info = res.data.info.detail.content;
                // 商品图片
                ...
                // 处理商品的描述信息
                this.content_after_sale = res.data.info.detail.after_sale;
                ...
                this.ruleForm.shop_cate_1 = '';
                this.shop_cate_list2 = [];
                this.ruleForm.shop_cate_2 = '';
                ...
                this.fileList = [];
                this.initial = true;
                this.content_info = '<p></p>';
                this.seo = {
                  title: '',
                  keywords: '',
                  description: ''
                };
                this.content_after_sale = '<p></p>'
              }
            }
            else {
              this.$message(res.data.msg)
      ...
      change_type() {
        if (this.initial && this.ruleForm.product_type) {
          ...
          this.axios.post(this.getApi('seller/auth/get_spec_attr'),
      ...
      // 根据第三级分类获取品牌
      getBrands(val) {
        this.axios.post(this.getApi('seller/auth/get_brands'),
          `id=${val}`
        ).then(res => {
          if (res.data.code === 0) {
      ...
      getShopCly() {
        this.axios.post(this.getApi('seller/auth/shop_cate_list')
          ...
          if (res.data.code === 0) {
            this.shop_cate_list = res.data.list
          }
        })
      },
      // 选择本店一级分类 筛选出二级分类
      changeShopCate(val) {
        this.shop_cate_list.forEach(item => {
          if (item.id == val) {
            this.shop_cate_list2 = item.childs
          }
        })
      },
      // 取消选择
      ...
      del_good(id) {
        this.$confirm('是否确认删除商品?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/goods_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
       ...
       *options 状态操作
       * val 0=是的操作  1=否的操作
       * tp 0=上下架 1=新品 2=热卖 3=推荐
       * id good_id
       */
      stateOperation(val, tp, id) {
        this.axios.post(this.getApi('seller/auth/goods_act'),
          this.dafa({
            status: val,
            type: tp,
            good_id: id
      ...
      changeSort(val, id) {
        if (/^\d*$/.test(val)) {
          this.axios.post(this.getApi('seller/auth/goods_sort'),
            this.dafa({
              sort: val,
              good_id: id
            })
      ...
      beforeImgUpload(file) {
        //console.log(file);
        //this.form.key = `${new Date().getTime()}_${file.name}`;
      ...
      reBuild: function (val, oldVal) {
        ...
        this.spec_list.forEach((item, index) => {
          let selectValues = item.selectedValues;
        ...
        for (let i = 0; i < ret.length; i++) {
          ...
          ret[i].forEach(item => {
            sku.specs.push({
              ...
              value: {id: item.id, name: item.name, sort: item.sort}
            })
          });
          vmSkus.push(sku)
        }
      ...
      submit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.antiShake.antiShake((resolve, reject) => {
                this.axios.post(this.getApi('seller/auth/goods_save'),
                  this.dafa({
                    id: this.id,
                    shop_fid_id: this.ruleForm.shop_fid_id,
                    gt_id: this.ruleForm.product_type,
                    cate_id: this.ruleForm.shop_cate_2,
                    ...
                    after_sale: this.content_after_sale,
                    imgs: pic,
                    img: pic[0],
                    specs: spec || [],
      ...
      // 图片上传成功回调
      on_suc(res, file, fileList, http) {
        this.is_img_up = true;
        this.imgs.push(res.key);
        // this.imageUrl = URL.createObjectURL(file.raw);
        // let addr = http ? `${http}/${res.key}` : res.key;
        // this.fileList.push({'url': addr});
      },
      //删除图片
...
//        console.log(this.del_pic);
    ...
    components: {
      hint, crumb, page, EditorItem, EditorsItem, uploadItem
    }
  }
</script>
