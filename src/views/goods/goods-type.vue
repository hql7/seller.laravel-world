            ...
            <tr v-for="(value, key) in brands">
              <td>
                ...
                <el-checkbox-group v-model="brand_checkList">
                  <el-checkbox v-for="ite in value" :key="ite.id" :label="ite.id">{{ite.name}}/{{ite.cg_name}}
                  </el-checkbox>
                </el-checkbox-group>
              </td>
            </tr>
    ...
    <el-dialog
      ...
      <el-form :model="dynamicValidateForm" ref="dynamicValidateForm" class="demo-dynamic">
        <el-form-item
          ...
          <el-input v-model="domain.name" :disabled="!!domain.id" placeholder="请填写属性值"></el-input>
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `商品类型是用来规定某一类商品共有规格和属性的集合，商品的属性为商品说明参数，仅供展示。商品的规格为商品购买时必须选择的参数`
        }, // 提示模块参数
        tableData: [], // 表格数据
    ...
    methods: {
      ...
      // 获取列表
      get_type() {
        this.axios.post(this.getApi('seller/auth/gt_list'),
          this.dafa({
      ...
      edit_type(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/gt_edit'),
            `id=${this.id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_attr(id, key) {
        let arr = [];
        if (id) {
          this.axios.post(this.getApi('seller/auth/gt_attr_del'),
            `id=${id}`
          ).then(res => {
            this.$message(res.data.msg);
            if (res.data.code === 0) {
      ...
      removeDomain(item) {
        if (item.id) {
          this.antiShake.antiShake((resolve, reject) => {
            this.axios.post(this.getApi('seller/auth/gt_value_del'),
              `id=${item.id}`
            ).then(res => {
              resolve(res);
              // this.$message(res.data.msg);
      ...
      del_type(id) {
        this.$confirm('是否确认删除此类型?', '提示', {
          ...
          this.axios.post(this.getApi('seller/auth/gt_del'),
            this.dafa({
              id_list: this.del_id
            })
          ).then(res => {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              this.axios.post(this.getApi('seller/auth/gt_save'),
                this.dafa({
                  id: this.id,
