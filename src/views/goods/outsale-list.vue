<template>
  <div class="goods-list prop-box">
    ...
    <div class="m-goods-list m-prop-box">
      ...
      <h2>
        <el-button type="info" @click="seeIllegal(1)">店铺下架商品</el-button>
        <el-button type="danger" @click="seeIllegal(2)">违规下架商品</el-button>
      </h2>
      <el-table
        v-loading="loading"
          ...
          <el-table-column key="1"
            ...
            <template slot-scope="scope">
              <el-switch
                ...
                :active-value="0"
                :inactive-value="1"
                :disabled="scope.row.is_online == 2">
              </el-switch>
            </template>
          </el-table-column>
        </template>
        <template v-else-if="type==2">
          <el-table-column key="2"
                           prop="down_reason"
                           label="违规禁售原因">
          </el-table-column>
          <el-table-column key="2"
                           label="重新申请审核">
            <template slot-scope="scope">
              <el-button type="primary" size="small" icon="edit" @click="reapply(scope.row.id)">
                申请
              </el-button>
            </template>
          </el-table-column>
        ...
        <el-tab-pane label="基本信息">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="240px">
            ...
            <!--<el-form-item label="商品分类" required class="col_box">
              ...
              &lt;!&ndash;<el-input class="u-ts" :disabled="true" value="最多成为第三级,如果设置为第二级, 只选择第一级即可"></el-input>&ndash;&gt;
            </el-form-item>-->
            <el-form-item label="经营类目商品" required class="col_box">
              <el-select v-model="ruleForm.shop_fid_id" placeholder="请选择经营类目">
  ...
  export default {
    data() {
      return {
        hint: {
          title: '操作提示',
          content: `商品只能添加在店铺二级分类上，请先保证店铺至少有一个二级分类！->在店铺管理-店铺分类！违规下架，当商品处于违规下架状态时，
          前台将不能购买该商品，只有管理员可控制商品违规下架状态，并且商品只有重新编辑后才能上架。上传商品时若选择不包邮将按照运费模版进行运费计算，
          请保证订单物流-》运费模版管理设置有一个默认运费模版。
          点击列表里的删除按钮删除单个管理员时不受复选框的影响！复选框数据只在点击列表下方批量删除时有效。
          商品名称进行搜索时为关键字搜索，货号必须全名称搜索！`
        }, // 提示模块参数
        category_id1: '', // 一级分类
        classify1: [], // 二级分类
        category_id2: '', // 二级分类
        classify2: [], // 二级分类
        shop_cate: null, // 本店分类 - 搜索
        shop_cate_list: [], // 本店分类盒子
        shop_cate_list2: [], // 本店2级分类盒子
        search_type: null,
        search: '', // 搜索内容
        type: 1, // 0=在售商品列表  1=下架商品列表 2=违规下架商品
        tableData: [], // 表格数据
        ...
        rules: {
          ...
          shop_fid_id: [
            {required: true, type: 'number', message: '请选择经营类目', trigger: 'blur'}
          ],
          shop_cate_2: [
    ...
    methods: {
      ...
      reapply(id) {
        this.antiShake.antiShake((resolve, reject) => {
          this.axios.post(this.getApi('seller/auth/goods_online_apply'),
            `id=${id}`
          ).then(res => {
            resolve(res);
            // this.$message(res.data.msg);
      ...
      reBuild: function (val, oldVal) {
        ...
        for (let i = 0; i < ret.length; i++) {
          ...
          sku.specs = [];
          ret[i].forEach(item => {
            sku.specs.push({propertyId: item.PropertyId, value: {id: item.id, name: item.name, sort: item.sort}})
          });
          vmSkus.push(sku)
        }
        // console.log(vmSkus)
...
//        console.log(this.del_pic);
    ...
    components: {
      hint, crumb, page, Editor, EditorS, uploadItem
    }
  }
</script>
